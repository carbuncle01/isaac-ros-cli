...
def local_copy_add_sources(instruction: str) -> List[str]:
    ...
    # Best-effort support for JSON-array COPY/ADD forms after options.
...
class ImageBuildPlan:
    ...
    def md5hash(self) -> str:
        ...
        for d in sorted(self.dockerfiles_, key=lambda x: x.image_key()):
            hash_md5.update(d.image_key().encode("utf-8") + b"\0")
            hash_md5.update(d.md5_hash().encode("ascii") + b"\0")
        return hash_md5.hexdigest()
